...
namespace glfw {
  namespace instance {
    namespace extensions {
      /// @descr Returns a vector containing the names of all the Vulkan Extensions required to create an instance
      vec<cstr> getReq(void) {
        u32 extensionCount = 0;
        cstr* extensions = glfw::vk::instance::getExts(&extensionCount);
        vec<cstr> result;
        for (u32 it = 0; it < extensionCount; it++) result.push_back(extensions[it]);
        return result;
      } //:: glfw.instance.extensions.getReq
...
namespace csys {
  namespace w {
    ...
    //____________________________
    void error (i32 code, const char* descr) { cdk::err(code, descr); }
