...
const private = struct {
  fn Flags (comptime T :type, comptime I :type) type {
    comptime std.debug.assert(@sizeOf(T) == @sizeOf(I));
    return struct {
      pub fn toInt   (v :T      ) I    { return @bitCast(v); }
      pub fn fromInt (v :I      ) T    { return @bitCast(v); }
      ...
      pub fn hasAll  (a :T, b :T) bool { return (toInt(a) & toInt(b)) == toInt(b); }
      pub fn hasAny  (a :T, b :T) bool { return (toInt(a) & toInt(b)) != 0; }
...
pub const InstanceCreate = packed struct {
  ...
  pub usingnamespace private.Flags(@This(), vk.Flags);
...
pub const Queue = packed struct {
  ...
  opticalFlow           :bool = false, // 07 :: VK_QUEUE_OPTICAL_FLOW_BIT_NV
...
pub const image = struct {
  pub const Usage = packed struct {
    ...
    invocationMaskHUAWEI          :bool = false,  // 18 :: VK_IMAGE_USAGE_INVOCATION_MASK_BIT_HUAWEI: c_int = 262_144;
    attachmentFeedbackLoop        :bool = false,  // 19 :: VK_IMAGE_USAGE_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT: c_int = 524_288;
    sampleWeightQCOM              :bool = false,  // 20 :: VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM: c_int = 1_048_576;
    sampleBlockMatchQCOM          :bool = false,  // 21 :: VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM: c_int = 2_097_152;
    hostTransfer                  :bool = false,  // 22 :: VK_IMAGE_USAGE_HOST_TRANSFER_BIT_EXT: c_int = 4_194_304;
    __reserved_bits_23_31 :u9=0,
    pub usingnamespace private.Flags(@This(), vk.Flags);

    pub const shadingRateImageNV = @This(){.fragmentShadingRateAttachment= true}; // VK_IMAGE_USAGE_SHADING_RATE_IMAGE_BIT_NV: c_int = 256;
