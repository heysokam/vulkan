...
const TODO = struct {
  ...
  pub const Swapchain = struct {
    ...
    pub fn recreate (S: *Swapchain, new_extent: vk.Extent2D) !void {
      const gc = S.gc;
      const allocator = S.allocator;
      const old_handle = S.handle;
      S.deinitExceptSwapchain();
      S.* = try initRecycle(gc, allocator, new_extent, old_handle);
    } //:: Swapchain.recreate
  ...
  pub fn main() !void {
    ...
    if (glfw.vk.supported() != glfw.True) {
    ...
    const window = glfw.window.create(@intCast(extent.width), @intCast(extent.height), app_name, null, null) orelse return error.WindowInitFailed;
    ...
    while (glfw.window.close(window) == glfw.False) {
      var w :c_int= undefined;
      var h :c_int= undefined;
      glfw.framebuffer.size(window, &w, &h);
